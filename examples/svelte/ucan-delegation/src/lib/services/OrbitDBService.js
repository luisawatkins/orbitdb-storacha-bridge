...
export class OrbitDBService {
  ...
  async forceDirectConnection() {
    ...
    // Check prerequisites
    if (!alice?.libp2p || !bob?.libp2p || !this.connectionState.alicePeerId || !this.connectionState.bobPeerId) {
      console.warn("⚠️ [FORCE_CONNECTION] Cannot force connection - missing libp2p instances or peer IDs");
      return false;
    }
    
    if (!this.connectionState.aliceAddressReady || !this.connectionState.bobAddressReady) {
    ...
    if (connectionEstablished) {
      console.log("🎉 [FORCE_CONNECTION] Strategy SUCCESS: Direct connection established!");
      return true;
    }
    ...
    while (!peersDiscovered && (Date.now() - discoveryStartTime) < discoveryTimeout) {
      const aliceConnectedToBob = this.connectionState.aliceConnectedPeers.includes(this.connectionState.bobPeerId);
      const bobConnectedToAlice = this.connectionState.bobConnectedPeers.includes(this.connectionState.alicePeerId);
  ...
  updatePeerConnectionStatus() {
    ...
    this.connectionState.peersConnected = this.connectionState.aliceConnectedPeers.length > 0 && this.connectionState.bobConnectedPeers.length > 0;
    ...
    if (this.connectionState.alicePeerId && this.connectionState.bobPeerId) {
      const directConnection = this.connectionState.aliceConnectedPeers.includes(this.connectionState.bobPeerId) || 
                              this.connectionState.bobConnectedPeers.includes(this.connectionState.alicePeerId);
      const commonPeers = this.connectionState.aliceConnectedPeers.some(peer => this.connectionState.bobConnectedPeers.includes(peer));
      this.connectionState.peersConnected = this.connectionState.peersConnected || directConnection || commonPeers;
    }
  ...
  async createOrbitDBInstance(
    ...
    const updateAddressReadiness = () => {
      ...
      console.log(`🔍 ${persona.toUpperCase()} current addresses:`, currentMultiaddrs);
      ...
      if (persona === "alice") {
        ...
        if (shouldBeReady && !wasReady) {
          ...
          this.addReplicationEvent({
            ...
            peer: 'alice',
            data: { addressCount: currentMultiaddrs.length, dialable: hasDialableAddresses }
          });
        }
      } else {
    ...
    const addressCheckInterval = setInterval(() => {
      const currentMultiaddrs = libp2p.getMultiaddrs().map(addr => addr.toString());
      const hasNewAddresses = JSON.stringify(currentMultiaddrs) !== JSON.stringify(
        persona === "alice" ? this.connectionState.aliceMultiaddrs : this.connectionState.bobMultiaddrs
      );
      
      if (hasNewAddresses) {
        console.log(`🔄 ${persona.toUpperCase()} addresses updated:`, currentMultiaddrs);
        updateAddressReadiness();
      }
      
      // Stop checking after address readiness is achieved
      const isReady = persona === "alice" ? this.connectionState.aliceAddressReady : this.connectionState.bobAddressReady;
      if (isReady) {
        clearInterval(addressCheckInterval);
        console.log(`✅ ${persona.toUpperCase()} address checking stopped - ready!`);
      }
    ...
    setTimeout(() => {
      ...
      console.log(`🚀 ${persona.toUpperCase()} forced ready for UCAN delegation demo`);
    }, 10000);
    ...
    libp2p.addEventListener('peer:connect', (event) => {
      ...
      if (persona === "alice") {
        this.connectionState.aliceConnectedPeers = [...this.connectionState.aliceConnectedPeers, peerId];
      } else {
        this.connectionState.bobConnectedPeers = [...this.connectionState.bobConnectedPeers, peerId];
      }
    ...
    libp2p.addEventListener('peer:disconnect', (event) => {
      ...
      if (persona === "alice") {
        this.connectionState.aliceConnectedPeers = this.connectionState.aliceConnectedPeers.filter(p => p !== peerId);
      } else {
        this.connectionState.bobConnectedPeers = this.connectionState.bobConnectedPeers.filter(p => p !== peerId);
      }
    ...
    await this.crossStoreIdentities(linkedIdentities, persona, sharedIdentity, bobIdentity);
    
    // Test identity resolution immediately after cross-storage (skip network-dependent tests)
    await this.testIdentityResolution(linkedIdentities, persona, sharedIdentity, bobIdentity);
    ...
    if (persona === "alice" && sharedIdentity) {
      ...
      console.log(`⚠️ ${persona} using default OrbitDB identity with IPFS-linked identities`);
    }
    ...
    if (openDatabase) {
      database = await this.createDatabase(orbitdb, persona, databaseName, databaseConfig, {
        sharedIdentity,
        bobIdentity,
   ...
   */
  async crossStoreIdentities(linkedIdentities, persona, sharedIdentity, bobIdentity) {
    ...
    if (sharedIdentity) {
      if (sharedIdentity.hash && sharedIdentity.bytes) {
        try {
          console.log(`   📦 Attempting to store Alice identity with hash: ${sharedIdentity.hash}`);
          ...
          if (currentPeer?.helia) {
            identityStorePromises.push(currentPeer.helia.blockstore.put(hashCID, sharedIdentity.bytes));
          }
        } catch (hashError) {
          console.error(`   ❌ Alice identity hash validation failed:`, hashError.message);
          ...
          try {
            ...
            console.log(`   ✅ Alice identity stored via identities.addIdentity()`);
          } catch (altError) {
            console.error(`   ❌ Alternative storage also failed:`, altError.message);
          }
        }
      } else {
        console.warn(`   ⚠️ Alice identity missing hash or bytes - cannot store in IPFS`);
        console.log(`     Hash: ${sharedIdentity.hash}, Bytes: ${!!sharedIdentity.bytes}`);
      }
    }
    ...
    if (bobIdentity) {
      if (bobIdentity.hash && bobIdentity.bytes) {
        try {
          console.log(`   📦 Attempting to store Bob identity with hash: ${bobIdentity.hash}`);
          ...
          const currentPeer = this.peers.get(persona);
          if (currentPeer?.helia) {
            identityStorePromises.push(currentPeer.helia.blockstore.put(hashCID, bobIdentity.bytes));
          }
        } catch (hashError) {
          console.error(`   ❌ Bob identity hash validation failed:`, hashError.message);
          console.log(`   🔧 Trying alternative storage method...`);
          
          // Alternative: Store using the identity directly through OrbitDB identities system
          try {
            await linkedIdentities.addIdentity(bobIdentity);
            console.log(`   ✅ Bob identity stored via identities.addIdentity()`);
          } catch (altError) {
            console.error(`   ❌ Alternative storage also failed:`, altError.message);
          }
        }
      } else {
        console.warn(`   ⚠️ Bob identity missing hash or bytes - cannot store in IPFS`);
        console.log(`     Hash: ${bobIdentity.hash}, Bytes: ${!!bobIdentity.bytes}`);
      }
    }
    ...
    if (identityStorePromises.length > 0) {
      try {
        ...
        console.log(`✅ Identity storage promises completed in ${persona}'s IPFS`);
      } catch (storageError) {
        console.error(`❌ Some identity storage failed in ${persona}'s IPFS:`, storageError);
        console.log(`   🔧 This might be expected if identities don't have proper hash/bytes`);
      }
    } else {
      console.log(`ℹ️ No IPFS storage needed - identities stored via alternative methods`);
    }
  }
   ...
   */
  async testIdentityResolution(linkedIdentities, persona, sharedIdentity, bobIdentity) {
    console.log(`🧪 Testing identity resolution for ${persona} after cross-storage:`);
    ...
    if (sharedIdentity) {
      ...
      if (sharedIdentity.hash) {
        try {
          ...
          const aliceByHashString = await linkedIdentities.getIdentity(sharedIdentity.hash);
          console.log(`     By hash (string): ${aliceByHashString ? '✅' : '❌'}`);
    ...
    if (bobIdentity) {
      ...
      if (bobIdentity.hash) {
        try {
          // Try both string hash and parsed CID
          const bobByHashString = await linkedIdentities.getIdentity(bobIdentity.hash);
          console.log(`     By hash (string): ${bobByHashString ? '✅' : '❌'}`);
   ...
   */
  async createDatabase(orbitdb, persona, databaseName, databaseConfig, options = {}) {
    ...
    if (persona === "alice") {
      ...
      console.log(`🆕 Alice creating new database with proper access control:`, databaseName);
      ...
      if (bobIdentity) {
        ...
        console.log(`🔐 Setting up database with write access for both Alice & Bob`);
      } else {
        console.log(`🔒 Setting up database with Alice-only access (Bob identity not ready yet)`);
      }
      ...
      console.log(`📍 Alice created shared database with address:`, database.address);
      ...
      console.log(`🔗 Bob opening existing database by Alice's address:`, sharedDatabaseAddress);
  ...
  setupDatabaseEventListeners(database, persona) {
    ...
    this.databases.set(database.address?.toString() || database.address, persona);
