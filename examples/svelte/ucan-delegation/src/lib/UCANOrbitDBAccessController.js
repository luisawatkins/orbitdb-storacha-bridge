...
/**
 * Creates an instance of UCANOrbitDBAccessController.
 * @callback UCANOrbitDBAccessController
...
/**
 * Defines an UCAN OrbitDB access controller.
 ...
 * @memberof module:AccessControllers
 */
const UCANOrbitDBAccessController =
  ({ write } = {}) =>
  async ({ orbitdb, identities, address, name }) => {
    ...
    const canAppend = async (entry) => {
      ...
      const hasWriteAccess =
        (await hasCapability("write", id)) ||
        (await hasCapability("admin", id));
      if (hasWriteAccess) {
