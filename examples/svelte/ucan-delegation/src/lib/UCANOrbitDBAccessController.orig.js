...
const UCANOrbitDBAccessController = (options = {}) => {
  ...
  return async ({ orbitdb, identities, address }) => {
    ...
    console.log(`   🔒 Access controller address: ${address || 'undefined'} (${typeof address})`)
    console.log(`   ⚡ UCAN features: ${storachaClient ? 'ENABLED' : 'DISABLED'}`)
    console.log('🔍 Identity debug info:')
    console.log(`   🆔 OrbitDB identity: ${orbitdb.identity?.id}`)
    console.log(`   🗂️ Identities instance: ${identities ? 'present' : 'missing'}`)
    ...
    if (address) {
      ...
      console.log(`   📝 Loading existing UCAN access controller from: ${address}`)
      try {
        ...
        if (value.write && Array.isArray(value.write)) {
          console.log(`   📋 Loading stored write permissions: [${value.write.join(', ')}]`)
          ...
          console.log(`   👑 Authority identity loaded from manifest: ${authorityIdentity}`)
        }
        console.log(`   ✅ Successfully loaded access controller manifest`)
      } catch (loadError) {
        console.warn(`   ⚠️ Could not load existing access controller manifest: ${loadError.message}`)
        console.log(`   💡 This is normal for new databases or when IPFS gateways are unavailable`)
        console.log(`   🚀 Continuing with initial write permissions: [${write.join(', ')}]`)
    ...
    console.log(`   🔍 Authority check: ${isAliceAuthority ? 'This is Alice (authority)' : 'This is Bob/other (participant)'}`)
    console.log(`   👑 Authority identity: ${authorityIdentity || 'undefined'}`)
    console.log(`   🆔 Current identity: ${orbitdb.identity.id}`)
    console.log(`   🔧 Authority loaded from: ${address ? 'stored manifest' : 'initial write parameter'}`)
    
    // Only create/modify accessDB if this is the authority (Alice) or if we need to read permissions
    if (address && address !== 'undefined' && address !== '/orbitdb/undefined') {
      ...
      try {
        ...
        try {
          ...
          console.log(`   🔍 DEBUG: AccessDB contains ${allEntries.length} total entries`)
          ...
          console.log(`   📋 Final: Loaded ${grantedWriteAccess.size} write permissions from storage`)
          console.log(`   📊 Final write permissions: [${Array.from(grantedWriteAccess).join(', ')}]`)
          console.log(`   📏 Loaded ${ucanDelegations.size} UCAN delegations from storage`)
          console.log(`   🚫 Loaded ${revocationProofs.size} revocations from storage`)
        } catch (readError) {
          console.warn(`   ⚠️ Could not read from access database: ${readError.message}`)
          console.log('   💡 This might be normal if no permissions have been granted yet')
        }
      } catch (error) {
        console.warn(`   ⚠️ Could not connect to access database: ${error.message}`)
        console.log('   📝 Continuing with in-memory-only mode')
        console.log('   💡 Alice will need to grant permissions for persistent storage')
      }
    } else {
      console.log('   📝 Address undefined - using in-memory-only mode for now')
    }
    
    console.log(`   ✅ UCAN access controller initialized ${accessDB ? 'with persistent storage' : 'in memory-only mode'}`)
    ...
    const canAppend = async (entry) => {
      ...
      try {
        ...
        console.log(`🧪 Testing identity resolution for hash: ${entry.identity}`);
        const writerIdentity = await identities.getIdentity(entry.identity)
        if (!writerIdentity) {
          console.log(`   ❌ Could not resolve identity from IPFS: ${entry.identity}`)
          console.log(`   🔧 This is likely the IPFS storage linkage issue!`)
          return false
        }
        console.log(`   ✅ Successfully resolved identity from IPFS hash: ${entry.identity}`);
    ...
    const grant = async (capability, identityId) => {
      ...
      if (!isAliceAuthority) {
        console.warn(`   🚫 PERMISSION DENIED: Only the database authority can grant access`)
        console.log(`   👑 Authority: ${write[0] || 'undefined'}`)
        console.log(`   🆔 Current identity: ${orbitdb.identity.id}`)
        console.log(`   💡 Only Alice can grant permissions to maintain UCAN delegation security`)
        return null
      }
      ...
      try {
        ...
        console.log(`   🔍 BEFORE GRANT: grantedWriteAccess size = ${grantedWriteAccess.size}`)
        console.log(`   🔍 BEFORE GRANT: contents = [${Array.from(grantedWriteAccess).join(', ')}]`)
        
        // Add to granted access set and persist it if possible
        grantedWriteAccess.add(identityId)
        
        // 🔍 DEBUG: Check state after adding to Set
        console.log(`   🔍 AFTER ADD: grantedWriteAccess size = ${grantedWriteAccess.size}`)
        console.log(`   🔍 AFTER ADD: contents = [${Array.from(grantedWriteAccess).join(', ')}]`)
        
        if (accessDB) {
          await accessDB.put(`write/${identityId}`, true)
          console.log(`   ✅ Added write permission for ${identityId} (persistent)`)
        } else {
          console.log(`   ✅ Added write permission for ${identityId} (memory-only)`)
        }
        ...
        if (storachaClient) {
          try {
            ...
            if (archive.ok) {
              ...
              console.log(`   ✅ Created revocable UCAN delegation for ${identityId}`)
    ...
    const revoke = async (capability, identityId, reason = 'Access revoked by administrator') => {
      ...
      if (!isAliceAuthority) {
        console.warn(`   🚫 PERMISSION DENIED: Only the database authority can revoke access`)
        console.log(`   👑 Authority: ${write[0] || 'undefined'}`)
        console.log(`   🆔 Current identity: ${orbitdb.identity.id}`)
        console.log(`   💡 Only Alice can revoke permissions to maintain UCAN delegation security`)
        return
      }
      ...
      try {
        ...
        if (accessDB) {
          ...
          console.log(`   ✅ Removed OrbitDB write permission for ${identityId} (persistent)`)
        } else {
          console.log(`   ✅ Removed OrbitDB write permission for ${identityId} (memory-only)`)
        }
        ...
        if (ucanDelegations.has(identityId)) {
          if (accessDB) {
            ...
            console.log(`   🗑️ Removed UCAN delegation record for ${identityId} (persistent)`)
          } else {
            console.log(`   🗑️ Removed UCAN delegation record for ${identityId} (memory-only)`)
          }
    ...
    console.log(`   🔧 Fix applied: Database address will be generated by OrbitDB (not /ucan/)`)
    ...
    const accessController = {
      ...
      get write() {
        ...
        console.log(`   🧠 grantedWriteAccess Set size: ${grantedWriteAccess.size}`)
        console.log(`   🔍 grantedWriteAccess contents: [${Array.from(grantedWriteAccess).join(', ')}]`)
