...
function createThemeStore() {
  ...
  return {
    ...
    set: (newTheme) => {
      ...
      logger.info({ theme: newTheme }, "Theme set to:");
      set(newTheme);
    },
    toggle: () => {
      let currentTheme;
      update((theme) => {
        ...
        logger.info({ theme: currentTheme }, "Theme toggled to:");
        return currentTheme;
      });
    },
    init: () => {
      const initialTheme = getInitialTheme();
      logger.info({ theme: initialTheme }, "Initializing theme:");
      ...
      mediaQuery.addEventListener("change", (e) => {
        if (!localStorage.getItem("carbon-theme")) {
          ...
          logger.info({ theme: newTheme }, "System theme changed to:");
          document.documentElement.setAttribute("theme", newTheme);
          document.documentElement.style.setProperty(
            "color-scheme",
            newTheme === "g100" ? "dark" : "light",
