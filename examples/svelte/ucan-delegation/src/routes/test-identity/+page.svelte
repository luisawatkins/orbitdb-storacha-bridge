<script>
  ...
  import { Button, Tile, InlineNotification, Loading, Grid, Row, Column } from 'carbon-components-svelte';
  ...
  async function runTests() {
    ...
    try {
      ...
      addResult('WebAuthn Support', 'success', `WebAuthn ${webAuthnStatus.supported ? 'supported' : 'not supported'}`, webAuthnStatus);

      // Test 2: Mnemonic Identity
      addResult('Mnemonic Identity', 'info', 'Creating mnemonic identity...');
      const mnemonicResult = await identityService.createMnemonicIdentity('test-alice');
      addResult('Mnemonic Identity', 'success', `Created identity: ${mnemonicResult.identity.id.slice(0, 32)}...`, {
      ...
      if (webAuthnStatus.supported) {
        addResult('WebAuthn Identity', 'info', 'Creating WebAuthn identity (will prompt for biometric)...');
        const webAuthnResult = await identityService.createWebAuthnIdentity('test-bob');
        addResult('WebAuthn Identity', 'success', `Created WebAuthn identity: ${webAuthnResult.identity.id.slice(0, 32)}...`, {
        ...
        addResult('WebAuthn Identity', 'info', 'Skipped - WebAuthn not supported in this browser');
      }

      // Test 4: Utility Functions
      addResult('Utility Functions', 'info', 'Testing utility functions...');
      const testSeed = identityService.generateMasterSeed('test mnemonic phrase', 'password');
      const seed32 = identityService.convertTo32BitSeed(testSeed);
      addResult('Utility Functions', 'success', 'All utility functions working', {
        masterSeedLength: testSeed.length,
        seed32Length: seed32.length
      });

      addResult('Complete', 'success', '🎉 All Identity Service tests completed successfully!');

    } catch (error) {
      addResult('Error', 'error', `Test failed: ${error.message}`, { error: error.stack });
    } finally {
      testing = false;
    }
  }
    ...
    <Column lg={12} md={16} sm={16}>
      <Tile>
        ...
        <p style="margin-bottom: 1rem; color: var(--cds-text-secondary);">
          This component tests the extracted Identity Service to ensure it works correctly 
              ...
              >
                {#if result.data}
                  <details style="margin-top: 0.5rem;">
                    ...
                    <pre style="font-size: 0.625rem; margin-top: 0.25rem; overflow-x: auto;">{JSON.stringify(result.data, null, 2)}</pre>
                  </details>
                {/if}
              </InlineNotification>
            {/each}
