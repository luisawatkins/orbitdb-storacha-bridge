...
async function testIdentityFix() {
  ...
  try {
    ...
    const { helia: aliceHelia, libp2p: aliceLibp2p } =
      await createHeliaInstance("-alice");

    // Create Bob's Helia instance
    console.log("🔧 Creating Bob's Helia instance...");
    const { helia: bobHelia, libp2p: bobLibp2p } =
      await createHeliaInstance("-bob");
    ...
    const aliceMasterSeed = generateMasterSeed(
      aliceSeedPhrase,
      "alice-password",
    );
    ...
    const aliceCanResolveAliceOld = await aliceLinkedOld.getIdentity(
      aliceIdentityOld.hash,
    );
    const aliceCanResolveBobOld = await aliceLinkedOld.getIdentity(
      bobIdentityOld.hash,
    );
    const bobCanResolveAliceOld = await bobLinkedOld.getIdentity(
      aliceIdentityOld.hash,
    );
    const bobCanResolveBobOld = await bobLinkedOld.getIdentity(
      bobIdentityOld.hash,
    );

    console.log(
      `   ❌ Alice resolves Alice: ${aliceCanResolveAliceOld ? "✅" : "❌"}`,
    );
    console.log(
      `   ❌ Alice resolves Bob: ${aliceCanResolveBobOld ? "✅" : "❌"} (expected fail)`,
    );
    console.log(
      `   ❌ Bob resolves Alice: ${bobCanResolveAliceOld ? "✅" : "❌"} (expected fail)`,
    );
    ...
    await aliceHelia.blockstore.put(
      aliceIdentityNew.hash,
      aliceIdentityNew.bytes,
    );
    await aliceHelia.blockstore.put(bobIdentityNew.hash, bobIdentityNew.bytes);
    await bobHelia.blockstore.put(
      aliceIdentityNew.hash,
      aliceIdentityNew.bytes,
    );
    ...
    const aliceCanResolveAliceNew = await aliceIdentitiesLinked.getIdentity(
      aliceIdentityNew.hash,
    );
    const aliceCanResolveBobNew = await aliceIdentitiesLinked.getIdentity(
      bobIdentityNew.hash,
    );
    const bobCanResolveAliceNew = await bobIdentitiesLinked.getIdentity(
      aliceIdentityNew.hash,
    );
    const bobCanResolveBobNew = await bobIdentitiesLinked.getIdentity(
      bobIdentityNew.hash,
    );

    console.log(
      `   ✅ Alice resolves Alice: ${aliceCanResolveAliceNew ? "✅" : "❌"}`,
    );
    console.log(
      `   ✅ Alice resolves Bob: ${aliceCanResolveBobNew ? "✅" : "❌"} (should work now!)`,
    );
    console.log(
      `   ✅ Bob resolves Alice: ${bobCanResolveAliceNew ? "✅" : "❌"} (should work now!)`,
    );
    ...
    console.log("\n🏆 RESULT:");
    console.log(
      `Cross-peer identity resolution: ${fixWorked ? "✅ FIXED!" : "❌ Still broken"}`,
    );

    if (fixWorked) {
      console.log("🎉 The IPFS identity linkage fix is working correctly!");
      console.log(
        "   Both Alice and Bob can now resolve each other's identities.",
      );
      console.log(
        "   This should resolve the OrbitDB replication access control issues.",
      );
    } else {
