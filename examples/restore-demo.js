    ...
    if (restoreResult.success) {
      ...
      try {
        ...
        if (restoredDB.type === "events") {
          ...
          logger.info(
            `   ✅ Total entries after test: ${updatedEntries.length}`,
          );
        } else {
          logger.info(
            `   ℹ️  Database type '${restoredDB.type}' - skipping write test`,
          );
        }
      } catch (error) {
      ...
      if (
        ...
        logger.info(
          "   • Make sure you have backed up a database to your Storacha space",
        );
        logger.info(
          "   • Try running backup-demo.js first to create a fresh backup",
        );
...
function showUsage() {
  ...
  logger.info(
    "  • Automatically discovers all backup files in your Storacha space",
  );
  logger.info(
