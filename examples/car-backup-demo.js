...
async function demoTimestampedBackups() {
  ...
  try {
    ...
    if (backup1.success) {
      logger.info(
        `✅ Backup v1 created at ${new Date(backup1.timestamp).toISOString()}`,
      );
    ...
    if (backup2.success) {
      logger.info(
        `✅ Backup v2 created at ${new Date(backup2.timestamp).toISOString()}`,
      );
...
async function demoRestoreSpecific() {
  ...
  try {
    ...
    if (backups.length < 2) {
      logger.info(
        "   Need at least 2 backups for this demo. Run demo 1 first!",
      );
      return;
    }
    ...
    if (restoreResult.success) {
      ...
      logger.info(
        `   Backup date: ${new Date(restoreResult.backupTimestamp).toISOString()}`,
      );
...
async function demoRestoreLatest() {
  ...
  try {
    ...
    if (restoreResult.success) {
      logger.info(
        `✅ Restored from backup: ${new Date(restoreResult.backupTimestamp).toISOString()}`,
      );
...
async function demoBackwardCompatibility() {
  ...
  try {
    ...
    if (legacyResult.success) {
      logger.info(
        `✅ Legacy backup: ${legacyResult.blocksUploaded} blocks uploaded`,
      );
    }
    ...
    if (carResult.success) {
      logger.info(
        `✅ CAR backup: ${carResult.blocksTotal} blocks in ${carResult.carFileSize} bytes`,
      );
      logger.info(
        `   More efficient: 2 files instead of ${legacyResult.blocksUploaded}`,
      );
    }
