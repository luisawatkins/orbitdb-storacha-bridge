...
async function testUCANListing() {
  ...
  try {
    ...
    logger.info(
      ...
      `   ✅ Delegation token: ${ucanToken.length} chars`,
    );
    ...
    try {
      ...
      logger.info(
        ...
        `   🎉 SUCCESS! Listed ${spaceFiles.length} files`,
      );

      if (spaceFiles.length > 0) {
        logger.info("\n📄 Files found:");
        spaceFiles.forEach((file, index) => {
          const uploadDate = file.uploaded
            ? new Date(file.uploaded).toLocaleString()
            : "Unknown";
          logger.info(
            { index: index + 1, root: file.root },
            `   ${index + 1}. ${file.root}`,
          );
        ...
        logger.info(
          ...
          `   Total files: ${spaceFiles.length}`,
        );
        logger.info(
          {
            totalSize: spaceFiles.reduce(
              (sum, f) => sum + (typeof f.size === "number" ? f.size : 0),
              0,
            ),
          },
          `   Total size: ${spaceFiles.reduce((sum, f) => sum + (typeof f.size === "number" ? f.size : 0), 0)} bytes`,
        );
      } else {
      ...
      logger.error(
        ...
        `   ❌ Listing failed: ${listError.message}`,
      );
      ...
      if (listError.message.includes("upload/list")) {
        ...
        logger.info(
          "   🤔 The UCAN delegation might not include upload/list capability",
        );
        logger.info(
          "   ✅ However, upload worked fine, so the delegation is valid",
        );
        logger.info("   💭 Options:");
        logger.info(
          "      1. Create new delegation with upload/list capability",
        );
