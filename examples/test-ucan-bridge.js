...
async function testUCANBridge() {
  ...
  try {
    ...
    logger.info(
      { tokenLength: ucanToken.length },
      `   ✅ Loaded delegation token (${ucanToken.length} chars)`,
    );
    ...
    try {
      ...
      logger.info(
        ...
        `   ✅ Successfully listed ${spaceFiles.length} files in space`,
      );
      ...
      if (spaceFiles.length > 0) {
        ...
        spaceFiles.slice(0, 3).forEach((file, i) => {
          logger.info(
            { index: i + 1, root: file.root, size: file.size },
            `      ${i + 1}. ${file.root} (${file.size} bytes)`,
          );
        });
      }
    } catch (error) {
      logger.warn(
        { error: error.message },
        `   ⚠️  Space listing failed: ${error.message}`,
      );
      logger.info(
        "   ℹ️  This might be expected if space is empty or has permission issues",
      );
    }
    ...
    const { helia, orbitdb, blockstore, datastore } =
      await createHeliaOrbitDB("-ucan-test");
    ...
    await db.put("test-key-2", {
      message: "Hello from UCAN!",
      timestamp: Date.now(),
    });
    ...
    try {
      ...
      if (backupResult.success) {
        ...
        logger.info(
          ...
          `   📍 Database: ${backupResult.databaseAddress}`,
        );
        logger.info(
          {
        ...
        logger.info(
          ...
          `   🔍 Block summary: ${JSON.stringify(backupResult.blockSummary)}`,
        );
      } else {
      ...
      try {
        ...
        logger.info(
          ...
          `   ✅ Found ${restoreFiles.length} files available for restore`,
        );

        if (restoreFiles.length > 0) {
          logger.info(
            "   ℹ️  Restore functionality verified (files available)",
          );
          logger.info(
            "   ⚠️  Skipping actual restore to avoid overwriting data",
          );
        }
      } catch (restoreError) {
        logger.warn(
          { error: restoreError.message },
          `   ⚠️  Restore file discovery failed: ${restoreError.message}`,
        );
      }
    } catch (backupError) {
      logger.error(
        { error: backupError.message, details: backupError },
        "   ❌ UCAN Backup failed",
      );
    }
...
function setupProgressTracking(bridge) {
  bridge.on("uploadProgress", (progress) => {
    if (progress.status === "starting") {
      logger.info(
        ...
        `   📤 Starting upload of ${progress.total} blocks...`,
      );
      ...
      if (percent % 20 === 0 || progress.current === progress.total) {
        ...
        logger.info(
          ...
          `   📊 Upload progress: ${progress.current}/${progress.total} (${percent}%)`,
        );
      }
    } else if (progress.status === "completed") {
      logger.info(
        {
          successful: progress.summary.successful,
          failed: progress.summary.failed,
        },
        `   ✅ Upload completed: ${progress.summary.successful} successful, ${progress.summary.failed} failed`,
      );
    }
  });
  ...
  bridge.on("downloadProgress", (progress) => {
    if (progress.status === "starting") {
      logger.info(
        { total: progress.total },
        `   📥 Starting download of ${progress.total} files...`,
      );
      ...
      if (percent % 20 === 0 || progress.current === progress.total) {
        logger.info(
          ...
          `   📊 Download progress: ${progress.current}/${progress.total} (${percent}%)`,
        );
      }
    } else if (progress.status === "completed") {
      logger.info(
        {
          downloaded: progress.summary.downloaded,
          failed: progress.summary.failed,
        },
        `   ✅ Download completed: ${progress.summary.downloaded} successful, ${progress.summary.failed} failed`,
      );
    }
  });
}
