...
function createLogger(config = {}) {
  ...
  if (process.env.NODE_ENV === 'development' || process.env.LOG_PRETTY === 'true') {
    return pino({
      ...loggerConfig,
      transport: {
        target: 'pino-pretty',
...
export const logUtils = {
  ...
  functionEntry: (functionName, params = {}, childLogger = logger) => {
    childLogger.debug({ function: functionName, params }, `Entering ${functionName}`);
  },
  ...
  functionExit: (functionName, result, childLogger = logger) => {
    childLogger.debug({ function: functionName, result }, `Exiting ${functionName}`);
  },
  ...
  timing: (operation, startTime, childLogger = logger) => {
    ...
    childLogger.info({ operation, duration }, `${operation} completed in ${duration}ms`);
  },
};
