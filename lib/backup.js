...
export async function backupDatabase(orbitdb, databaseAddress, options = {}) {
  ...
  console.log("🚀 Starting OrbitDB Database Backup")
  ...
  try {
    ...
    try {
      ...
      await stream.pipe(await createWritableCarReader({
      ...
      try {
        ...
        console.warn(`Failed to clean up metadata after error: ${cleanupError.message}`)
      }
...
export async function restoreFromSpace(orbitdb, options = {}) {
  ...
  console.log("🔄 Starting OrbitDB Restore")
  ...
  try {
    ...
    let metadata
    try {
      const metadataJson = await Signer.readFile(`${spaceName}/${latestBackup.metadata}`)
    ...
    try {
      // Get blocks stream
      const carStream = await Signer.readStream(`${spaceName}/${latestBackup.blocks}`)
      ...
      for await (const block of reader.blocks()) {
        try {
          ...
          console.warn(`⚠️ Failed to restore block: ${blockError.message}`)
        }
      }
