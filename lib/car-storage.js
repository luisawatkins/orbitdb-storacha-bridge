...
const CARStorage = async ({ path, name = 'orbitdb-data', autoFlush = true, flushThreshold = 100 } = {}) => {
  ...
  const loadFromCAR = async () => {
    try {
      ...
      if (stats.isFile()) {
        console.log(`Loading existing CAR file: ${primaryCarPath}`)
        ...
        console.log(`Loaded ${cache.size} entries from CAR file`)
      }
    } catch (error) {
      if (error.code !== 'ENOENT') {
        console.warn(`Error loading CAR file: ${error.message}`)
      }
    }
  }
  ...
  const flushToCAR = async () => {
    ...
    console.log(`Flushing ${cache.size} entries to CAR file`)
    ...
    try {
      ...
      for (const [key, value] of cache.entries()) {
        try {
          ...
          console.warn(`Error processing entry ${key}:`, error.message)
        }
      }
      ...
      if (blocks.length > 0) {
        ...
        console.log(`Successfully flushed to CAR file: ${primaryCarPath}`)
      }
    } catch (error) {
      console.error(`Error flushing to CAR file: ${error.message}`)
      throw error
    }
  }
