...
export async function cleanupAllTestArtifacts() {
  ...
  try {
    const entries = await fs.promises.readdir('.', { withFileTypes: true })
    const carFiles = entries.filter(entry => 
      entry.isFile() && entry.name.endsWith('.car')
    )
  ...
  console.log(`🧹 Comprehensive cleanup completed: ${cleanedDirs} directories, ${cleanedFiles} CAR files`)
}
  ...
  try {
    ...
    if (carPrefix) {
      try {
        ...
        console.warn(`⚠️ Error cleaning CAR files with prefix ${carPrefix}: ${error.message}`)
      }
    }
  } catch (error) {
    if (error.code !== 'ENOENT') {
      console.warn(`⚠️ Could not clean up test directory ${testDir}: ${error.message}`)
    }
  }
}
