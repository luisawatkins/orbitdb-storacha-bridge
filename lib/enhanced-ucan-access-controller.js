...
import { Verifier, Signer as UnifiedSigner } from '@ucanto/principal'
 ...
 */
const getRecipientFromOrbitDBIdentity = async (identityId, orbitDBIdentity = null) => {
...
const EnhancedUCANAccessController = (options = {}) => {
  ...
  return async ({ orbitdb, identities, address }) => {
    ...
    const accessDB = await orbitdb.open(accessControllerAddress, {
      type: 'keyvalue',
      AccessController: () => async ({ orbitdb, identities, address }) => ({
