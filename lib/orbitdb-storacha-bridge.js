...
export async function extractDatabaseBlocks(database, options = {}) {
  ...
  const extractionMode = logEntriesOnly ? 'log entries only (fallback mode)' : 'all blocks'
  
  console.log(`🔍 Extracting ${extractionMode} from database: ${database.name}`)
  ...
  if (!logEntriesOnly) {
      ...
      try {
        ...
        if (cid.code === 0x71) { // dag-cbor codec
          ...
          if (content.id && content.type) {
            ...
            console.log(`   ✓ Identity block found: ${hash}${referencedIdentities.has(hash) ? ' (referenced)' : ' (discovered)'}`)
          }
        }
      } catch (error) {
    ...
    console.log(`   ⚡ Skipping manifest, access controller, and identity blocks (fallback mode)`)
  }
 ...
 */
async function uploadBlocksToStoracha(blocks, client, batchSize = 10, maxConcurrency = 3, eventEmitter = null) {
  console.log(`📤 Uploading ${blocks.size} blocks to Storacha in batches of ${batchSize}...`)
...
export async function removeLayerFiles(layer, cids, options = {}) {
  ...
  console.log(`   🗑️ Removing ${cids.length} files from ${layer} layer using SDK (batch size: ${batchSize})...`)
      ...
      for (const result of batchResults) {
        if (result.status === 'fulfilled') {
          if (result.value.success) {
            ...
            failed++
            console.log(`         ❌ Failed to remove ${result.value.cid}: ${result.value.error}`)
          }
        } else {
          failed++
    ...
    try {
      ...
      if (cid.code === 0x71) { // dag-cbor codec
        try {
          ...
          } else if (content.type === 'orbitdb-access-controller' || content.type === 'ipfs') {
...
export async function backupDatabase(orbitdb, databaseAddress, options = {}) {
  ...
  const backupMode = logEntriesOnly ? 'Log Entries Only (Fallback Mode)' : 'Full Backup'
  ...
  try {
    ...
    const { successful, cidMappings } = await uploadBlocksToStoracha(blocks, client, 10, 3, eventEmitter)
...
export async function restoreLogEntriesOnly(orbitdb, options = {}) {
  ...
  try {
    ...
    const logEntryBlocks = await downloadLogEntriesOnly(spaceFiles, orbitdb, config, eventEmitter)
    
    if (logEntryBlocks.size === 0) {
      throw new Error('No log entries found in Storacha space')
    }
    
    console.log(`   ⚡ OPTIMIZATION: Downloaded only ${logEntryBlocks.size} log entries instead of ${spaceFiles.length} total files`)
    ...
    const fallbackResult = await reconstructWithoutManifest(orbitdb, logEntryBlocks, config)
    
    console.log('✅ Optimized Log-Entries-Only Restore completed successfully!')
    ...
    return {
      ...
      optimizationSavings: {
        ...
        percentageSaved: Math.round(((spaceFiles.length - logEntryBlocks.size) / spaceFiles.length) * 100)
      }
    }
    
  } catch (error) {
    console.error('❌ Optimized Log-Entries-Only Restore failed:', error.message)
...
export async function restoreDatabaseFromSpace(orbitdb, options = {}) {
  ...
  try {
    ...
    const downloadedBlocks = await downloadBlocksWithProgress(spaceFiles, orbitdb, config, eventEmitter)
    ...
    // const inferredDatabaseType = inferDatabaseType(analysis.logEntryBlocks)
    // console.log(`   🔍 Inferred database type: ${inferredDatabaseType}`)
    // const reconstructedDB = await orbitdb.open(databaseAddress, { type: inferredDatabaseType })
    const reconstructedDB = await orbitdb.open(config.dbName, config.dbConfig)
    ...
    // No cleanup needed - using orbitdb parameter directly
...
export async function restoreDatabase(orbitdb, manifestCID, cidMappings = null, options = {}) {
  ...
  try {
    ...
    console.log('✅ Restore completed successfully!', reconstructedEntries.length)
 ...
 */
async function downloadLogEntriesOnly(spaceFiles, currentOrbitDB, config, eventEmitter = null) {
      ...
      if (parsedCID.code === 0x71) {
        try {
          ...
          if (content && 
            ...
            logEntryBlocks.set(orbitdbCID, { storachaCID, bytes: bytes.length })
            logEntriesFound++
  ...
  console.log(`   📊 Found ${logEntriesFound} log entries out of ${totalFiles} total files`)
 ...
 */
async function downloadBlocksWithProgress(spaceFiles, currentOrbitDB, config, eventEmitter = null) {
...
async function reconstructWithoutManifest(orbitdb, downloadedBlocks, config) {
  ...
  for (const [cidString, _] of downloadedBlocks) {
    try {
      ...
      console.warn(`   ⚠️ Error processing block ${cidString}: ${error.message}`)
    }
  }
  ...
  if (logEntries.length === 0) {
    throw new Error('No OrbitDB log entries found in blocks - cannot reconstruct database')
  }
  
  // Step 2: Analyze payload patterns to determine database type
  console.log('🔍 Step 2: Analyzing payload patterns to determine database type...')
  ...
  for (const entry of logEntries) {
    try {
      ...
      console.warn(`   ⚠️ Failed to import entry ${entry.hash.slice(0, 12)}...: ${error.message}`)
    }
  }
  
  console.log(`   📊 Import complete: ${importedCount}/${logEntries.length} entries imported`)
...
function inferDatabaseType(logEntries) {
  ...
  for (const entry of logEntries) {
    ...
    if (payload && typeof payload === 'object') {
      ...
      if (payload.op === 'PUT' && payload.key && payload.value !== undefined) {
        if (payload.key.startsWith('_id')) {
          payloadPatterns.hasDocumentOps++
        } else {
          payloadPatterns.hasKeyValueOps++
        }
      } else if (payload.op === 'COUNTER') {
  ...
  if (payloadPatterns.hasCounterOps > 0) {
    return 'counter'
  } else if (payloadPatterns.hasDocumentOps > payloadPatterns.hasKeyValueOps && 
             payloadPatterns.hasDocumentOps > payloadPatterns.hasSimplePayloads) {
    return 'documents'
  } else if (payloadPatterns.hasKeyValueOps > payloadPatterns.hasSimplePayloads) {
    return 'keyvalue'
  } else {
...
async function importEntryByType(database, entry, databaseType) {
  ...
  switch (databaseType) {
    ...
    case 'keyvalue':
      if (payload && payload.op === 'PUT' && payload.key && payload.value !== undefined) {
        await database.set(payload.key, payload.value)
      } else {
