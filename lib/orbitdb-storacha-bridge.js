...
/**
 * Mapping-independent restore from Storacha with optional restart (ENHANCED)
 * 
 * @param {Object} orbitdb - Target OrbitDB instance
 * @param {Object} options - Restore options
 * @param {boolean} options.restartAfterRestore - Whether to restart OrbitDB/Helia after adding blocks (default: false)
...
export async function restoreDatabaseFromSpace(orbitdb, options = {}) {
  ...
  try {
    ...
    const downloadedBlocks = await downloadBlocksWithProgress(spaceFiles, currentOrbitDB, config, eventEmitter)
    
    // ... rest of existing code remains the same ...
    
    // Step 3: Intelligent block analysis
    console.log('\n🔍 Step 3: Analyzing block structure with advanced intelligence...')
    const analysis = await analyzeBlocks(currentOrbitDB.ipfs.blockstore, downloadedBlocks)
    ...
    if (analysis.manifestBlocks.length === 0 || options.forceFallback) {
      ...
      const fallbackResult = await reconstructWithoutManifest(currentOrbitDB, downloadedBlocks, config)
    ...
    const reconstructedDB = await currentOrbitDB.open(databaseAddress)
    ...
    return {
      ...
      orbitdb: currentOrbitDB, // Return the current (potentially restarted) OrbitDB instance
    ...
    // Cleanup if restart was attempted but failed
    if (cleanupFunction) {
      try {
        await cleanupFunction()
      } catch (cleanupError) {
        console.warn('⚠️ Cleanup after failure warning:', cleanupError.message)
      }
    }
