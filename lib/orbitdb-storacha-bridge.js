  ...
  if (useJoinEntry) {
    ...
    for (const entry of logEntries) {
      try {
        ...
        if (updated) {
          joinedCount++;
          console.log(`   ✓ Joined entry ${joinedCount}/${logEntries.length}: ${entry.hash.slice(0, 12)}...`);
        } else {
          // Entry might already be in the log
          console.log(`   → Entry already processed: ${entry.hash.slice(0, 12)}...`);
        }
      } catch (error) {
        joinErrors.push({ entry: entry.hash, error: error.message });
        console.warn(
    ...
    console.log(`   📊 Join complete: ${joinedCount}/${logEntries.length} entries joined`);
    importedCount = joinedCount;
    importErrors = joinErrors;
