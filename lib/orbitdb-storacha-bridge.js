...
/**
 * Execute w3 CLI command (DEPRECATED: Use SDK equivalents instead)
 * 
 * @param {Array} args - Command arguments
 * @returns {Promise<Object>} - Command result
 * @deprecated Use SDK methods like listStorachaSpaceFiles() instead
 */
export async function executeW3Command(args) {
  console.warn('⚠️ executeW3Command is deprecated. Use SDK equivalents instead.')
...
export async function listStorachaSpaceFiles(options = {}) {
  ...
  try {
    ...
    const client = await initializeStorachaClient(
      options.storachaKey || process.env.STORACHA_KEY,
      options.storachaProof || process.env.STORACHA_PROOF
    )
