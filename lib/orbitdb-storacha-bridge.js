...
export async function extractDatabaseBlocks(database, options = {}) {
  ...
  if (!logEntriesOnly) {
    ...
    console.log(`   🔍 Getting identity blocks from identities system and log entries...`);
    ...
    console.log(`   🔍 Scanning remaining storage blocks for missed identities...`);
...
export async function restoreDatabaseFromSpace(orbitdb, options = {}) {
  ...
  try {
    ...
    // CRITICAL FIX: Use joinEntry to properly traverse log and find heads
    console.log("   🔗 Step 4a: Using joinEntry to properly traverse log entries...");
    let joinedEntries = 0;
    
    // Get all log entry blocks and join them properly
    for (const logEntryBlock of analysis.logEntryBlocks) {
      try {
        // Create an entry object that matches OrbitDB's expected format
        // The entry should have both hash and the content properties at the same level
        const entryData = {
        ...
        if (updated) {
          joinedEntries++;
          console.log(`   ✓ Joined entry ${joinedEntries}: ${logEntryBlock.cid.slice(0, 12)}...`);
        } else {
          // Entry might already be in the log or was processed differently
          console.log(`   → Entry already processed: ${logEntryBlock.cid.slice(0, 12)}...`);
        }
      } catch (joinError) {
        console.warn(`   ⚠️ Failed to join entry ${logEntryBlock.cid}: ${joinError.message}`);
        // Continue with other entries even if one fails
      }
    }
    
    console.log(`   📊 Successfully joined ${joinedEntries}/${analysis.logEntryBlocks.length} entries`);
    ...
    console.log(`   🔗 Log entries joined: ${joinedEntries}`);
 ...
 */
async function reconstructWithoutManifest(orbitdb, downloadedBlocks, config, useJoinEntry = false) {
...
// Helper functions for backup filenames
...
/**
 * Find latest backup files in a space
 * 
 * @param {Array} spaceFiles - List of files in the space
 * @returns {{metadata: string, blocks: string}|null} - Latest backup filenames or null if none found
 */
function findLatestBackup(spaceFiles) {
  const backupFiles = spaceFiles
    .map(f => f.root.toString())
    .filter(f => f.match(/backup-.*-(metadata|blocks)\./)) // Find backup files
  ...
  if (!latestMetadata || !latestBlocks) {
    return null
  }
...
export {
  ...
  // Export backup filename helpers
