...
/**
 * Upload blocks to Storacha with parallel batch processing
 * 
 ...
 */
async function uploadBlocksToStoracha(blocks, client, batchSize = 10, maxConcurrency = 3) {
...
export async function restoreDatabaseFromSpace(orbitdb, options = {}) {
  ...
  try {
    ...
    if (analysis.manifestBlocks.length === 0) { //TODO if no manifest we need to decode the blocks and extract the payload and import as new entries (identify if it is document, key-value, etc)
    ...
    // const reconstructedDB2 = await orbitdb.open(databaseAddress)
    ...
    console.log('   ⏳ Waiting for entries to load...')
    await new Promise(resolve => setTimeout(resolve, config.timeout / 10))
    // const reconstructedEntries2 = await reconstructedDB2.all()
