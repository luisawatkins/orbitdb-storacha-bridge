...
function findCorrectManifest(analysis) {
  ...
  for (const logEntry of analysis.logEntryBlocks) {
    if (logEntry.content && logEntry.content.id) {
      ...
      logger.info(`   📝 Log entry references database: ${logEntry.content.id} (manifest: ${manifestCID})`)
    }
  }
  
  logger.info(`   🔍 Found ${databaseIds.size} unique database ID(s) from ${analysis.logEntryBlocks.length} log entries`)
  ...
  for (const manifestBlock of analysis.manifestBlocks) {
    ...
    logger.info(`   📋 Manifest ${manifestBlock.cid}: ${count > 0 ? 'MATCHES' : 'no match'}`)
  }
  ...
  if (bestManifest && bestCount > 0) {
    logger.info(`   ✅ Selected manifest: ${bestManifest.cid} (referenced by ${bestCount} log entries)`)
    return bestManifest
  }
  
  // Fallback: if no manifest matches log entries, use the first one and warn
  logger.warn('   ⚠️ No manifest matched log entries, using first manifest as fallback')
  return analysis.manifestBlocks[0]
}
...
async function loadUCANFromFile(ucanFilePath) {
  try {
    ...
    logger.info(`   📋 Capabilities: ${extractedDelegation.capabilities.map(cap => cap.can).join(', ')}`)
...
async function loadUCANFromToken(ucanToken) {
  try {
    ...
    try {
      ...
      logger.info(`   ⚠️  @ucanto/core failed: ${ucantoError.message.substring(0, 100)}...`)
    }
    ...
    try {
      ...
      logger.info(`   ⚠️  w3up-client/proof failed: ${proofError.message.substring(0, 100)}...`)
    }
...
async function uploadBlocksToStorachaUCAN(blocks, client, batchSize = 10, maxConcurrency = 3, eventEmitter = null) {
  logger.info(`📤 Uploading ${blocks.size} blocks to Storacha (UCAN) in batches of ${batchSize}...`)
  ...
  for (let i = 0; i < blocksArray.length; i += batchSize * maxConcurrency) {
    ...
    logger.info(`   🔄 Processing ${batches.length} concurrent batches (${megaBatch.length} blocks)...`)
    
    const batchPromises = batches.map(async (batch, batchIndex) => {
      logger.info(`     📦 Batch ${batchIndex + 1}/${batches.length}: ${batch.length} blocks`)
...
export async function restoreDatabaseFromSpaceWithUCAN(orbitdb, options = {}) {
  ...
  try {
    ...
    logger.info(`   🎉 SUCCESS! Found ${spaceFiles.length} files in space using UCAN authentication`)
    ...
    if (analysis.manifestBlocks.length === 0 || options.forceFallback) {
      logger.info('⚠️ No manifest blocks found - attempting fallback reconstruction...')
    ...
    logger.info(`   🎯 Selected manifest: ${correctManifest.cid} (matched from log entries)`)
...
// Helper function for fallback reconstruction (imported from main bridge)
async function reconstructWithoutManifest(_orbitdb, _downloadedBlocks, _config) { // Prefixed unused params with underscore
  // This would use the existing implementation from the main bridge
  // For brevity, I'm not reimplementing it here
  throw new Error('Fallback reconstruction not yet implemented in UCAN bridge')
}
