...
/**
 * Discover log entry blocks from backup CID mappings
 */
async function discoverLogEntries(backupResult) {
  console.log('🔍 Discovering log entry blocks from backup CID mappings...')
  
  const discoveredBlocks = new Map()
  
  try {
    // Extract all OrbitDB CIDs from backup mappings (excluding manifest and access controller)
    const mappings = backupResult.cidMappings || {}
    const manifestCID = backupResult.manifestCID
    const accessControllerCID = null // We'll identify this by parsing manifest
    
    // Find all CIDs that are not manifest or access controller
    const logAndIdentityCIDs = Object.keys(mappings).filter(cid => {
      return cid !== manifestCID && !cid.includes('access') // Basic filtering
    })
    
    console.log(`   📋 Found ${logAndIdentityCIDs.length} potential log/identity blocks from CID mappings`)
    
    for (const cidStr of logAndIdentityCIDs) {
      console.log(`   🔍 Processing block: ${cidStr}`)
      
      // Download the block
      const bytes = await downloadBlockFromStoracha(cidStr, mappings)
      if (bytes) {
        const cid = CID.parse(cidStr)
        discoveredBlocks.set(cidStr, { cid, bytes })
        console.log(`   ✅ Added block: ${cidStr} (${bytes.length} bytes)`)
      }
    }
    
  } catch (error) {
    console.warn(`   ⚠️ Error discovering blocks: ${error.message}`)
  }
...
async function testImprovedManifestDrivenApproach() {
  ...
  try {
    ...
    const logBlocks = await discoverLogEntries(backupResult)
