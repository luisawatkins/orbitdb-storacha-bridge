...
describe('Timestamped backups error handling', () => {
  ...
  test('should migrate old backup format', async () => {
    ...
    const sourceDB = await sourceNode.orbitdb.open('test-old-format', { type: 'events' })
  ...
  test('should handle multiple invalid backups and use latest valid one', async () => {
    // Create and populate test database
    const sourceDB = await sourceNode.orbitdb.open('test-multiple', { type: 'events' })
    await sourceDB.add('Test entry')

    // Create first valid backup
    const backup1 = await backupDatabase(sourceNode.orbitdb, sourceDB.address, AUTH)
    ...
    const newestTimestamp = new Date(Date.now() + 1000).toISOString().replace(/[:.]/g, '-')
    await Signer.writeFile(
  ...
  test('should warn about orphaned files', async () => {
    ...
    const otherTimestamp = new Date(Date.now() + 1000).toISOString().replace(/[:.]/g, '-')
    await Signer.writeFile(
