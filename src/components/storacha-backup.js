 ...
 */
export async function createStorachaIdentityFromSeed(seedPhrase, password = 'password', derivationIndex = 1) {
  try {
 ...
 */
export async function initializeStorachaClientWithSeed(seedPhrase, password = 'password', delegationToken = null) {
  try {
    ...
    const store = new StoreMemory()
    const client = await Client.create({ principal: identity.principal, store })
    ...
    if (delegationToken) {
      ...
      console.log(`✅ Storacha client initialized with seed + delegation: ${space.did()}`)
    } else {
      console.log(`✅ Storacha client initialized with seed identity: ${identity.did}`)
...
export async function createStorachaAccount(email) {
  try {
    ...
    return {
      success: false,
      message: 'Account creation requires manual setup. Please visit web3.storage to create an account and get your credentials.',
...
export async function createSpace(client, spaceName) {
  try {
    ...
    if (accounts.length === 0) {
      throw new Error('No accounts found - space creation requires account-based authentication')
    }
